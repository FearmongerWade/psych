...
{
	final path:String = 'backgrounds/limo/';
	...
	{
		var skyBG:BGSprite = new BGSprite('${path}limoSunset', -120, -50, 0.1, 0.1);
		add(skyBG);
		...
		if(!Settings.data.lowQuality) {
			...
			bgLimo = new BGSprite('${path}bgLimo', -150, 480, 0.4, 0.4, ['background limo pink'], true);
			add(bgLimo);
			...
			//PRECACHE SOUND
			Paths.sound('song/limo/dancerdeath');
			setDefaultGF('gf-car');
		}

		fastCar = new BGSprite('${path}fastCarLol', -300, 160);
		fastCar.active = true;
	}
	override function createPost()
	{
		...
		var limo:BGSprite = new BGSprite('${path}limoDrive', -120, 550, 1, 1, ['Limo stage'], true);
				...
				case KILLING:
					...
					for (i in 0...dancers.length) {
						if(dancers[i].x < FlxG.width * 1.5 && limoLight.x > (370 * i) + 170) {
							switch(i) {
								case 0 | 3:
									if(i == 0) FlxG.sound.play(Paths.sound('song/limo/dancerdeath'), 0.5);

									var diffStr:String = i == 3 ? ' 2 ' : ' ';
									var particle:BGSprite = new BGSprite('${path}gore/noooooo', dancers[i].x + 200, dancers[i].y, 0.4, 0.4, ['hench leg spin' + diffStr + 'PINK'], false);
									grpLimoParticles.add(particle);
									var particle:BGSprite = new BGSprite('${path}gore/noooooo', dancers[i].x + 160, dancers[i].y + 200, 0.4, 0.4, ['hench arm spin' + diffStr + 'PINK'], false);
									grpLimoParticles.add(particle);
									var particle:BGSprite = new BGSprite('${path}gore/noooooo', dancers[i].x, dancers[i].y + 50, 0.4, 0.4, ['hench head spin' + diffStr + 'PINK'], false);
									grpLimoParticles.add(particle);

									var particle:BGSprite = new BGSprite('${path}gore/stupidBlood', dancers[i].x - 110, dancers[i].y + 20, 0.4, 0.4, ['blood'], false);
									particle.flipX = true;
									particle.angle = -57.5;
									grpLimoParticles.add(particle);
								case 1:
	...
	{
		//trace('Car drive');
		FlxG.sound.play(Paths.soundRandom('song/limo/carPass', 0, 1), 0.7);
