...
class PhillyBlazin extends BaseStage
{
	...
	override function create()
	{
		...
		{
			var skyImage = Paths.image('${path}phillyBlazin/skyBlur');
			...
			skyAdditive = new BGSprite('${path}phillyBlazin/skyBlur', -600, -175, 0.0, 0.0);
			setupScale(skyAdditive);
			skyAdditive.visible = false;
			add(skyAdditive);
			
			lightning = new BGSprite('${path}phillyBlazin/lightning', -50, -300, 0.0, 0.0, ['lightning0'], false);
			setupScale(lightning);
			lightning.visible = false;
			add(lightning);
		}
		
		var phillyForegroundCity:BGSprite = new BGSprite('${path}phillyBlazin/streetBlur', -600, -175, 0.0, 0.0);
		setupScale(phillyForegroundCity);
		add(phillyForegroundCity);
		
		if(!Settings.data.lowQuality)
		{
			foregroundMultiply = new BGSprite('${path}phillyBlazin/streetBlur', -600, -175, 0.0, 0.0);
		...
		if(_song.gameOverSound == null || _song.gameOverSound.trim().length < 1) GameOverSubstate.deathSoundName = 'song/weekend/fnf_loss_sfx-pico-gutpunch';
	...
	function precache()
	{
		for (i in 1...4)
		{
			Paths.sound('song/weekend/lightning/Lightning$i');
		}
	}
	...
	function applyLightning():Void
	{
		...
		// Sound
		FlxG.sound.play(Paths.soundRandom('song/weekend/lightning/Lightning', 1, 3));
	}
