...
class SchoolEvil extends BaseStage
{
	...
	override function create()
	{
		...
		if(!Settings.data.lowQuality)
			bg = new BGSprite('${path}animatedEvilSchool', posX, posY, 0.8, 0.9, ['background 2'], true);
		else
			bg = new BGSprite('${path}animatedEvilSchool_low', posX, posY, 0.8, 0.9);
		...
		FlxG.sound.playMusic(Paths.music('song/weeb/LunchboxScary'), 0);
	...
	override function eventPushed(event:objects.Note.EventNote)
	{
		...
		switch(event.event)
		{
			case "Trigger BG Ghouls":
				if(!Settings.data.lowQuality)
				{
					bgGhouls = new BGSprite('${path}bgGhouls', -100, 190, 0.9, 0.9, ['BG freaks glitch instance'], false);
	...
	function schoolIntro():Void
	{
		...
		senpaiEvil.frames = Paths.getSparrowAtlas('${path}senpaiCrazy');
			...
			if (doof != null)
			{
				...
				new FlxTimer().start(0.3, function(swagTimer:FlxTimer)
				{
					...
					else
					{
						...
						FlxG.sound.play(Paths.sound('song/weeb/Senpai_Dies'), 1, false, null, true, function()
						{
							remove(senpaiEvil);
							senpaiEvil.destroy();
							remove(red);
