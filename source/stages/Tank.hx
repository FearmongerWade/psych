...
{
	final path:String = 'backgrounds/tank/';
	...
	override function create()
	{
		...
		if(!Settings.data.lowQuality) foregroundSprites.add(new BGSprite('${path}tank1', -300, 750, 2, 0.2, ['fg']));
		foregroundSprites.add(new BGSprite('${path}tank2', 450, 940, 1.5, 1.5, ['foreground']));
		if(!Settings.data.lowQuality) foregroundSprites.add(new BGSprite('${path}tank4', 1300, 900, 1.5, 1.5, ['fg']));
		foregroundSprites.add(new BGSprite('${path}tank5', 1620, 700, 1.5, 1.5, ['fg']));
		if(!Settings.data.lowQuality) foregroundSprites.add(new BGSprite('${path}tank3', 1300, 1200, 3.5, 2.5, ['fg']));
	...
	function prepareCutscene()
	{
		...
		Paths.loadAnimateAtlas(tankman, '${path}cutscenes/tankman');
	...
	function ughIntro()
	{
		...
		cutsceneHandler.music = 'song/tankman/DISTORTO';
		Paths.sound('song/tankman/wellWellWell');
		Paths.sound('song/tankman/killYou');
		Paths.sound('song/tankman/bfBeep');

		var wellWellWell:FlxSound = new FlxSound().loadEmbedded(Paths.sound('song/tankman/wellWellWell'));
		FlxG.sound.list.add(wellWellWell);
		var killYou:FlxSound = new FlxSound().loadEmbedded(Paths.sound('song/tankman/killYou'));
		...
		cutsceneHandler.timer(4.5, function()
		{
			...
			FlxG.sound.play(Paths.sound('song/tankman/bfBeep'));
		});
	...
	function gunsIntro()
	{
		...
		cutsceneHandler.music = 'song/tankman/DISTORTO';
		Paths.sound('song/tankman/tankSong2');

		var tightBars:FlxSound = new FlxSound().loadEmbedded(Paths.sound('song/tankman/tankSong2'));
	...
	function stressIntro()
	{
		...
		Paths.sound('song/tankman/stressCutscene');

		pico = new FlxAnimate(gf.x + 150, gf.y + 450);
		pico.showPivot = false;
		Paths.loadAnimateAtlas(pico, '${path}cutscenes/picoAppears');
		...
		var cutsceneSnd:FlxSound = new FlxSound().loadEmbedded(Paths.sound('song/tankman/stressCutscene'));
