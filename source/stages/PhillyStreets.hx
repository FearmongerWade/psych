...
class PhillyStreets extends BaseStage
{
	...
	override function create()
	{
		...
		{
			var skyImage = Paths.image('${path}phillyStreets/phillySkybox');
			...
			var phillySkyline:BGSprite = new BGSprite('${path}phillyStreets/phillySkyline', -545, -273, 0.2, 0.2);
			add(phillySkyline);
			darkenable.push(phillySkyline);

			var phillyForegroundCity:BGSprite = new BGSprite('${path}phillyStreets/phillyForegroundCity', 625, 94, 0.3, 0.3);
			add(phillyForegroundCity);
			darkenable.push(phillyForegroundCity);
		}

		var phillyConstruction:BGSprite = new BGSprite('${path}phillyStreets/phillyConstruction', 1800, 364, 0.7, 1);
		add(phillyConstruction);
		darkenable.push(phillyConstruction);

		var phillyHighwayLights:BGSprite = new BGSprite('${path}phillyStreets/phillyHighwayLights', 284, 305, 1, 1);
		add(phillyHighwayLights);
		darkenable.push(phillyHighwayLights);

		if(!Settings.data.lowQuality)
		{
			var phillyHighwayLightsLightmap:BGSprite = new BGSprite('${path}phillyStreets/phillyHighwayLights_lightmap', 284, 305, 1, 1);
		...
		var phillyHighway:BGSprite = new BGSprite('${path}phillyStreets/phillyHighway', 139, 209, 1, 1);
		add(phillyHighway);
		darkenable.push(phillyHighway);

		if(!Settings.data.lowQuality)
		{
			var phillySmog:BGSprite = new BGSprite('${path}phillyStreets/phillySmog', -6, 245, 0.8, 1);
			add(phillySmog);
			darkenable.push(phillySmog);

			for (i in 0...2)
			{
				var car:BGSprite = new BGSprite('${path}phillyStreets/phillyCars', 1200, 818, 0.9, 1, ['car1', 'car2', 'car3', 'car4'], false);
				add(car);
				switch(i)
				{
			...
			phillyTraffic = new BGSprite('${path}phillyStreets/phillyTraffic', 1840, 608, 0.9, 1, ['redtogreen', 'greentored'], false);
			add(phillyTraffic);
			darkenable.push(phillyTraffic);

			var phillyTrafficLightmap:BGSprite = new BGSprite('${path}phillyStreets/phillyTraffic_lightmap', 1840, 608, 0.9, 1);
		...
		var phillyForeground:BGSprite = new BGSprite('${path}phillyStreets/phillyForeground', 88, 317, 1, 1);
		...
		if(_song.gameOverSound == null || _song.gameOverSound.trim().length < 1) GameOverSubstate.deathSoundName = 'song/weekend/fnf_loss_sfx-pico';
		if(_song.gameOverLoop == null || _song.gameOverLoop.trim().length < 1) GameOverSubstate.loopSoundName = 'song/weekend/gameOver-pico';
		if(_song.gameOverEnd == null || _song.gameOverEnd.trim().length < 1) GameOverSubstate.endSoundName = 'song/weekend/gameOverEnd-pico';
	...
	function darnellCutscene()
	{
		...
		var cutsceneMusic:FlxSound = new FlxSound().loadEmbedded(Paths.music('song/weekend/darnellCanCutscene'));
		cutsceneMusic.looped = true;
		FlxG.sound.list.add(cutsceneMusic);

		var darnellLaugh:FlxSound = new FlxSound().loadEmbedded(Paths.sound('song/weekend/cutscene/darnell_laugh'));
		darnellLaugh.volume = 0.6;
		FlxG.sound.list.add(darnellLaugh);

		var neneLaugh:FlxSound = new FlxSound().loadEmbedded(Paths.sound('song/weekend/cutscene/nene_laugh'));
		...
		cutsceneHandler.timer(cutsceneDelay + 5.1, function() //pico fires at can
		{
			...
			FlxG.sound.play(Paths.soundRandom('song/weekend/shots/shot', 1, 4));
	...
	function precache()
	{
		...
		function createCan()
		{
			...
			lightCanSnd.loadEmbedded(Paths.sound('song/weekend/Darnell_Lighter'));
			
			kickCanSnd = new FlxSound();
			FlxG.sound.list.add(kickCanSnd);
			kickCanSnd.loadEmbedded(Paths.sound('song/weekend/Kick_Can_UP'));

			kneeCanSnd = new FlxSound();
			FlxG.sound.list.add(kneeCanSnd);
			kneeCanSnd.loadEmbedded(Paths.sound('song/weekend/Kick_Can_FORWARD'));
			didCreateCan = true;
		}
		...
		function precacheCasing()
		{
			...
			if(!Settings.data.lowQuality)
			{
				casingFrames = Paths.getSparrowAtlas('${path}PicoBullet'); //precache
			...
			gunPrepSnd.loadEmbedded(Paths.sound('song/weekend/Gun_Prep'));
		...
		for (noteType in noteTypes)
		{
			switch(noteType)
			{
				...
				case 'weekend-1-firegun':
					...
					bonkSnd.loadEmbedded(Paths.sound('song/weekend/Pico_Bonk'));
			}
		}
		...
		for (i in 1...5)
			Paths.sound('song/weekend/shots/shot$i');
	}
	...
	override function noteMiss(note:Note)
	{
		switch(note.noteType)
		{
			case 'weekend-1-firegun':
				...
				{
					GameOverSubstate.deathSoundName = 'song/weekend/fnf_loss_sfx-pico-explode';
					GameOverSubstate.loopSoundName = 'song/weekend/gameOverStart-pico-explode';
